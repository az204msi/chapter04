...
class BookValidationTests {
    ...
    void whenIsbnNotDefinedThenValidationFails() {
        var now = Instant.now();
        var book = Book.of("", "Title", "Author", 9.90, now, now);
        Set<ConstraintViolation<Book>> violations = validator.validate(book);
    ...
    void whenIsbnDefinedButIncorrectThenValidationFails() {
        var now = Instant.now();
        var book = Book.of("a234567890", "Title", "Author", 9.90, now, now);
        Set<ConstraintViolation<Book>> violations = validator.validate(book);
    ...
    void whenTitleIsNotDefinedThenValidationFails() {
        var now = Instant.now();
        var book = Book.of("1234567890", "", "Author", 9.90, now, now);
        Set<ConstraintViolation<Book>> violations = validator.validate(book);
        assertThat(violations).hasSize(1);
        assertThat(violations.iterator().next().getMessage())
    ...
    void whenAuthorIsNotDefinedThenValidationFails() {
        var now = Instant.now();
        var book = Book.of("1234567890", "Title", "", 9.90, now, now);
        Set<ConstraintViolation<Book>> violations = validator.validate(book);
        assertThat(violations).hasSize(1);
        assertThat(violations.iterator().next().getMessage())
    ...
    void whenPriceIsNotDefinedThenValidationFails() {
        var now = Instant.now();
        var book = Book.of("1234567890", "Title", "Author", null, now, now);
        Set<ConstraintViolation<Book>> violations = validator.validate(book);
        assertThat(violations).hasSize(1);
        assertThat(violations.iterator().next().getMessage())
    ...
    void whenPriceDefinedButZeroThenValidationFails() {
        var now = Instant.now();
        var book = Book.of("1234567890", "Title", "Author", 0.0, now, now);
        Set<ConstraintViolation<Book>> violations = validator.validate(book);
        assertThat(violations).hasSize(1);
        assertThat(violations.iterator().next().getMessage())
